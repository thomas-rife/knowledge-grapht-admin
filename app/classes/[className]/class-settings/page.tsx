...
import {
  deleteClass,
  createClassCode,
} from "@/app/classes/[className]/class-settings/actions";
...
const ClassSettings = ({
  ...
  const handleSettingChange = (setting: string, value: boolean) => {
    ...
    showSnackbar(
      `Setting updated: ${setting} is now ${value ? "enabled" : "disabled"}`,
    );
  };
  ...
  const saveClassName = () => {
    ...
    showSnackbar(
      "Updated class name is not persistent yet. This is a placeholder message.",
    ); // Placeholder message until server action is implemented
    ...
    <Box sx={{ padding: 3 }}>
      <Paper elevation={2} sx={{ padding: 3, marginBottom: 4, height: "100%" }}>
                    ...
                    >
                      ...
                      >
                        {timeLeft === null
                          ...
                          : timeLeft > 60
                            ? `Expires in ${Math.floor(timeLeft / 60 / 60 / 24 / 7)} weeks`
        ...
        <Box sx={{ marginTop: 4 }}>
          <Paper
            elevation={0}
            sx={{ padding: 3, border: "1px solid #ffdddd", borderRadius: 1 }}
          >
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            This will permanently delete <strong>{currentClassName}</strong> and
            all associated data, including lessons, assignments, and student
