...
> => {
  ...
  // 1) Resolve class_id by name ONCE with validation
  ...
  if (classRows.length > 1) {
    console.error('⚠️ Multiple classes found with name:', cleanedClassName, classRows)
    return []
  }
  ...
  if (!lessonLinks || lessonLinks.length === 0) {
    console.error('No lesson found for name:', cleanedLessonName, 'in class_id:', classId)
    return []
  }
  if (lessonLinks.length > 1) {
    console.error('⚠️ Multiple lessons found with name:', cleanedLessonName, lessonLinks)
    return []
  }
...
export const getAllowedTopics = async (className: string, lessonName?: string) => {
  const supabase = createClient()
  ...
  // Return stable list
  return Array.from(allowed).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }))
}
...
export async function createNewQuestion(
  ...
  if ((profCount ?? 0) === 0) {
    ...
    return { success: false, error: 'Not authorized. Your account is not in professor_courses.' }
  }
  ...
  const normalizedImageUrl =
    typeof (questionData as any)?.image_url === 'string' && (questionData as any).image_url.trim()
  ...
  if (questionData.questionType === 'multiple-choice') {
    ...
    const { questionType, prompt, snippet, answerOptions, answer } = multipleChoiceData

    // Coerce topics to a non-null string[]
    const rawTopics =
      (multipleChoiceData as any)?.topics ?? (multipleChoiceData as any)?.topicsCovered ?? []
    ...
    const { questionType, prompt, snippet, answerOptions, answer } = rearrangeData
    ...
    // Coerce topics to a non-null string[]
    const rawTopics = (rearrangeData as any)?.topics ?? (rearrangeData as any)?.topicsCovered ?? []
    const safeTopics: string[] = Array.isArray(rawTopics)
      ? rawTopics.filter(t => typeof t === 'string' && t.trim() !== '')
      : []
  ...
  if (clsRowsCQ.length > 1) {
    console.error('Multiple class rows match name. Use a unique slug or ID:', cleanedClassName)
    return { success: false, error: 'Multiple classes match this name. Use a unique slug or ID.' }
  }
  ...
  if (lessonIDError) {
    console.error('Error fetching lesson ID through class_lesson_bank: ', lessonIDError)
    return { success: false, error: lessonIDError }
  }
  if (!link) {
    console.error('No lesson match for: ', cleanedLessonName, 'in class_id:', classID.class_id)
...
export const deleteQuestionFromLesson = async (
  ...
  try {
    ...
    const cleanedLessonName = decodeURIComponent(lessonName).replace(/-/g, ' ').trim()
    ...
    if (!links || links.length === 0) return { success: false, error: 'Lesson not found for class' }
    if (links.length > 1) {
      console.warn('⚠️ Multiple lessons matched for delete:', cleanedLessonName, links)
...
export async function updateQuestion(id: number, questionData: Question) {
  ...
  // Coerce topics to a non-null string[]
  const rawTopics = (topics as any) ?? (questionData as any)?.topicsCovered ?? []
  const safeTopics: string[] = Array.isArray(rawTopics)
    ? rawTopics.filter(t => typeof t === 'string' && t.trim() !== '')
    : []
...
export async function deleteQuestion(className: string, lessonName: string, questionId: number) {
  const supabase = createClient()
  ...
  if (classRows.length !== 1) {
    console.error('Could not uniquely resolve class for deleteQuestion:', cleanedClassName)
...
export async function generateQuestionLLM(params: {
  ...
  if (clsRowsLLM.length > 1) {
    console.error('generateQuestionLLM: multiple classes matched', cleanedClassName)
    return { success: false, error: 'Multiple classes match this name. Use a unique slug or ID.' }
  }

  const class_id = clsRowsLLM[0].class_id
  const level = typeof clsRowsLLM[0].level === 'number' ? clsRowsLLM[0].level : 0
  ...
  if (!resp.ok) {
    ...
    console.error('generateQuestionLLM: LLM endpoint failed', resp.status, preview)
  ...
  const normalizedOptions = Array.isArray(llmQ?.answer_options)
    ? llmQ.answer_options.map((v: any, i: number) => ({ [`option${i + 1}`]: String(v ?? '') }))
    : []
