...
async function getClassIdByName(supabase: ReturnType<typeof createClient>, raw: string) {
  const cleaned = decodeURIComponent(raw).replace(/%20/g, ' ').replace(/-/g, ' ').trim()
  ...
  return data?.[0]?.class_id as number | undefined
}
...
export const createNewLesson = async (
  ...
  try {
    const PLACEHOLDER_LABELS = ['Edit me 😊!', 'Edit me', 'Edit Me 😊!', 'Edit Me']

    const nodesArray: string[] = Array.isArray((graphData as any).nodes)
      ? ((graphData as any).nodes as any[]).map(v => String(v ?? '').trim()).filter(Boolean)
      : []
    if (nodesArray.length === 1 && PLACEHOLDER_LABELS.includes(nodesArray[0])) {
      console.error('createNewLesson: only placeholder node exists (nodes array)')
      return {
        success: false,
        error:
    ...
    const flow =
      Array.isArray(react_flow_data) && react_flow_data[0] ? (react_flow_data[0] as any) : null
    ...
    if (flowLabels.length === 1 && PLACEHOLDER_LABELS.includes(flowLabels[0])) {
      console.error('createNewLesson: only placeholder node exists (react_flow_data)')
      return {
        success: false,
        error:
          'You must edit your class graph first (rename or add topics) before creating lessons.',
  ...
  const normalizedTopics = Array.isArray(topics)
    ? Array.from(new Set(topics.map(t => (typeof t === 'string' ? t.trim() : '')).filter(Boolean)))
    : []

  const PLACEHOLDER_LABELS = ['Edit me 😊!']
  if (normalizedTopics.length === 1 && PLACEHOLDER_LABELS.includes(normalizedTopics[0])) {
    console.error('createNewLesson: submitted single placeholder topic')
    return {
      success: false,
      error: 'Please rename your default topic in the class graph before creating a lesson.',
    }
  }

  if (normalizedTopics.length === 0) {
    console.error('createNewLesson: cannot create a lesson without at least one topic')
    return { success: false, error: 'A lesson must include at least one topic.' }
  }
  ...
  await supabase.from('class_lesson_bank').insert({
    owner_id: user.id,
    class_id: classId,
...
export const updateLesson = async (
  ...
  if (normalizedTopics.length === 0) {
    console.error('updateLesson: cannot save a lesson without at least one topic')
    return { success: false, error: 'A lesson must include at least one topic.' }
  }
